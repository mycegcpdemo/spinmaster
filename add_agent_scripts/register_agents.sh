...
# It creates the curls commands but do not execute them.
...
echo "--- Reading credentials.json to prepare commands... ---"
...
echo "--- Fetching Project Number (required for the agent body)... ---"
# We perform this lookup now so the output command has the correct static number
...
# URL Encode the Redirect URI and Scopes for the complex authorizationUri string
...
echo "### COMMAND 1: Create Authorization Resource ###"
echo ""
cat <<EOF
curl -X POST \\
...
echo "### COMMAND 2: Register ADK Agent ###"
echo ""
cat <<EOF
curl -X POST \\
   -H "Authorization: Bearer \$(gcloud auth print-access-token)" \\
